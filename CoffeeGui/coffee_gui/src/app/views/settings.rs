...
pub fn create(store: &mut Store) -> View {
    ...
    let page_left: Box<Button>         = Box::new(Button::new("◀".to_string(), 0, 36, 16, 22, 1, 5, 15, Event::new("[pager.click]", Some(vec!["left".to_string()])), &CONFIG));
    let page_right: Box<Button>         = Box::new(Button::new("▶".to_string(), 233, 36, 16, 22, 1, 5, 15, Event::new("[pager.click]", Some(vec!["right".to_string()])), &CONFIG));
